/**
 * Which-Key Extension (local fixed copy)
 *
 * Shows available keybindings in a searchable panel.
 * Trigger with Ctrl+/ or /which-key.
 *
 * Fixed: action names now match the namespaced keys returned by
 * getEffectiveConfig() (e.g. "tui.editor.cursorUp", "app.interrupt").
 */
...
const CATEGORIES: { name: string; actions: string[] }[] = [
  ...
  { name: "Selection", actions: ["tui.select.up", "tui.select.down", "tui.select.pageUp", "tui.select.pageDown", "tui.select.confirm", "tui.select.cancel"] },
