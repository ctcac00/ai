...
#   ./install.sh --self         # also install agentic-workflow plugin (off by default)
...
for a in "$@"; do
  case "$a" in
    ...
    *) echo "error: unknown argument '$a'" >&2; usage >&2; exit 2 ;;
  esac
done
...
section() {
  ...
  printf '\033[2m%.0s─\033[0m' {1..50}; printf '\n'
}
...
install_dir() {
  local src="$1" dst="$2"
  [ -d "$src" ] || { warn "missing source $src — skip"; return 0; }
  ...
  if [ "$OVERWRITE_COUNT" -gt 0 ]; then
    confirm_overwrite "$dst" || { skip "$dst — skipped (not confirmed)"; return 0; }
  fi
  mkdir -p "$dst"
...
ensure_rtk() {
  ...
  if curl -fsSL "$RTK_INSTALL_URL" | sh; then
    ...
    command -v rtk >/dev/null 2>&1 && ok "rtk $(rtk --version)" || { warn "rtk install finished but rtk not on PATH"; return 1; }
  else
...
do_claude() {
  local src dst
  src="$(src_for claude)"; dst="$(dst_for claude)"
...
do_codex() {
  local src dst
  src="$(src_for codex)"; dst="$(dst_for codex)"
...
do_omp() {
  local src dst
  src="$(src_for omp)"; dst="$(dst_for omp)"
  subsection "omp  →  $dst"
  install_dir "$src" "$dst"
}
...
do_opencode() {
  local src dst
  src="$(src_for opencode)"; dst="$(dst_for opencode)"
...
if [ "$RTK" -eq 1 ]; then
  ...
  for h in "${TARGETS[@]}"; do
    ...
    case "$_seen_rtk_flags" in
      *"|${_f}|"*) skip "rtk init $h — same flag as pi, duplicate skipped" ; continue ;;
    esac
