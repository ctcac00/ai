    ...
    for path in md_files:
        ...
        for target in collect_link_targets(path):
            ...
            if t.startswith("/"):
                ...
                abs_target = (path.parent / t).resolve()
                if not abs_target.is_relative_to(bundle.resolve()):
                    continue  # points outside the bundle — not a bundle cross-link
                resolved = abs_target.relative_to(bundle.resolve()).as_posix()
